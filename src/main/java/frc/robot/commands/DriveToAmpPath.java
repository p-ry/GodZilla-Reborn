...
public class DriveToAmpPath extends Command {
 ...
 if(!validTarget){
    ...
    if (validTarget) {
      ...
      if (shiftDirection == 1) {
        where =  Utilitys.shiftPoseRight(Utilitys.getAprilTagPose(tagId), 60, 16.5); // 0.164285833);
      }
      if (shiftDirection == -1) {
        where = Utilitys.shiftPoseLeft(Utilitys.getAprilTagPose(tagId), 60, 16.5);// 0.164285833);
      }
    }
      //
